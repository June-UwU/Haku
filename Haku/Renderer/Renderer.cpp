...
void D3D12HelloTriangle::OnInit(HWND handle)
{
	LoadPipeline(handle);
	LoadAssets();
}
...
void D3D12HelloTriangle::LoadPipeline(HWND handle)
{
	...
	HAKU_SOK_ASSERT(CreateDXGIFactory2(dxgiFactoryFlags, IID_PPV_ARGS(&factory)))

	if (false) // subbing literal value
	{
		Microsoft::WRL::ComPtr<IDXGIAdapter> warpAdapter;
		HAKU_SOK_ASSERT(factory->EnumWarpAdapter(IID_PPV_ARGS(&warpAdapter)))

		HAKU_SOK_ASSERT(D3D12CreateDevice(warpAdapter.Get(), D3D_FEATURE_LEVEL_11_0, IID_PPV_ARGS(&m_device)))
	}
	else
	{
		...
		HAKU_SOK_ASSERT(D3D12CreateDevice(hardwareAdapter.Get(), D3D_FEATURE_LEVEL_11_0, IID_PPV_ARGS(&m_device)))
	}
	...
	HAKU_SOK_ASSERT(m_device->CreateCommandQueue(&queueDesc, IID_PPV_ARGS(&m_commandQueue)));
	...
	HAKU_SOK_ASSERT(factory->CreateSwapChainForHwnd(
	...
	HAKU_SOK_ASSERT(factory->MakeWindowAssociation(handle, DXGI_MWA_NO_ALT_ENTER));

	HAKU_SOK_ASSERT(swapChain.As(&m_swapChain));
	...
	// Create descriptor heaps.
	{
		...
		HAKU_SOK_ASSERT(m_device->CreateDescriptorHeap(&rtvHeapDesc, IID_PPV_ARGS(&m_rtvHeap)));
	...
	// Create frame resources.
	{
		...
		{
			HAKU_SOK_ASSERT(m_swapChain->GetBuffer(n, IID_PPV_ARGS(&m_renderTargets[n])));
...
void D3D12HelloTriangle::LoadAssets()
{
	...
	// Create the vertex buffer.
	{
		...
		VertexData triangleVertices[] = { { { 0.0f, 0.25f, 0.0f }, { 1.0f, 0.0f, 0.0f, 1.0f } },
...
void D3D12HelloTriangle::GetHardwareAdapter(
...
void D3D12HelloTriangle::WaitForPreviousFrame()
{
	...
	HAKU_SOK_ASSERT(m_commandQueue->Signal(m_fence.Get(), fence));
	m_fenceValue++;

	// Wait until the previous frame is finished.
	if (m_fence->GetCompletedValue() < fence)
	{
		HAKU_SOK_ASSERT(m_fence->SetEventOnCompletion(fence, m_fenceEvent));
