...
{
	HAKU_LOG_INFO("Creating LLR Device");
	unsigned int FactoryFlags{};
#ifdef _DEBUG
	{
		Microsoft::WRL::ComPtr<ID3D12Debug> DebugDevice;
	...
	HAKU_SOK_ASSERT(D3D12CreateDevice(GraphicsAdapter.Get(), D3D_FEATURE_LEVEL_12_0, IID_PPV_ARGS(&m_Device)))
}
RenderDevice::~RenderDevice()
{
	ShutDown();
}
void RenderDevice::ShutDown() noexcept
{
	m_Device->Release();
}
} // namespace Renderer
