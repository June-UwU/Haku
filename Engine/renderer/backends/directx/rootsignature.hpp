 ...
 * \return H_OK on sucess and H_FAIL on fail
 */
i8 add_root_descriptor_srv(directx_root_signature* signature , 
	const u64 register_space = 0, D3D12_ROOT_DESCRIPTOR_FLAGS flags = D3D12_ROOT_DESCRIPTOR_FLAG_NONE);
 ...
 * \return H_OK on sucess and H_FAIL on fail
 */
i8 add_root_descriptor_uav(directx_root_signature* signature,
	const u64 register_space = 0, D3D12_ROOT_DESCRIPTOR_FLAGS flags = D3D12_ROOT_DESCRIPTOR_FLAG_NONE);
 ...
 * \return H_OK on sucess and H_FAIL on fail
 */
i8 add_root_descriptor_cbv(directx_root_signature* signature,
	const u64 register_space = 0, D3D12_ROOT_DESCRIPTOR_FLAGS flags = D3D12_ROOT_DESCRIPTOR_FLAG_NONE);
 ...
 * \return H_OK on sucess and H_FAIL on fail
 */
i8 add_table_parameter(directx_root_signature* signature, darray* table,descriptor_type type,const u64 number_of_descriptor,
	const u64 register_space = 0,D3D12_DESCRIPTOR_RANGE_FLAGS flags = D3D12_DESCRIPTOR_RANGE_FLAG_NONE);

//i8 bind_root_signature(directx_root_signature* signature);
